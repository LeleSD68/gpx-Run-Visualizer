...
const GeminiTrackAnalysisPanel: React.FC<GeminiTrackAnalysisPanelProps> = ({ stats, userProfile, track }) => {
    ...
    const handleAnalyze = useCallback(async () => {
        ...
        const generatePrompt = () => {
             const profileInfo = `Dati dell'atleta:
- Età: ${userProfile.age ?? 'N/A'}
- FC Massima: ${userProfile.maxHr ?? 'N/A'}`;
            ...
            const hrContext = stats.avgHr ? `
Dati sulla Frequenza Cardiaca:
- Frequenza Cardiaca Media: ${Math.round(stats.avgHr)} bpm (Max: ${stats.maxHr ?? 'N/A'} bpm)
- Distribuzione nelle Zone di FC (basata su una FC Max di ${hrZoneInfo.maxHrUsed} bpm):
${hrDistribution}
` : '';
